...
public final class StudentDashboardApiController extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    StudentDashboardService studentDashboardService = AppComponentFactory.studentDashboardService(getServletContext());

    try {
      RoleSessionDto actor = requireActor(request);
      StudentDashboardDto dashboard = studentDashboardService.getDashboard(actor.userId());
      jsonResponseWriter.writeApiResponse(
          response,
          HttpServletResponse.SC_OK,
          ApiResponse.success("Student dashboard loaded successfully.", dashboard));
